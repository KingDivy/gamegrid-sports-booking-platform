...
const STATUS_BADGE = {
  Open: 'badge-green', Active: 'badge-blue',
  Closed: 'badge-gray', Full: 'badge-orange', Cancelled: 'badge-red',
};

export default function EventCard({ event }) {
  const navigate  = useNavigate();
  const spotsLeft = event.maxTeams - (event.registeredTeamsCount || 0);
  const fillPct   = Math.min(100, ((event.registeredTeamsCount || 0) / event.maxTeams) * 100);
    ...
    <div className="event-card animate-fade" onClick={() => navigate(`/events/${event._id}`)}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: '1.1rem', fontWeight: 700 }}>{event.registeredTeamsCount}/{event.maxTeams}</div>
        </div>
      </div>

      {/* Neumorphic progress bar */}
      <div style={{ marginTop: 12, borderRadius: 6, height: 8, boxShadow: 'inset 3px 3px 6px #b8bec7, inset -3px -3px 6px #ffffff' }}>
        <div style={{
          height: 8, borderRadius: 6,
          background: 'linear-gradient(90deg, var(--accent-primary), var(--accent-secondary))',
          width: `${fillPct}%`,
          transition: 'width 0.6s ease',
          boxShadow: '2px 0 8px rgba(74,124,247,0.3)',
        }} />
      </div>
    </div>
  );
